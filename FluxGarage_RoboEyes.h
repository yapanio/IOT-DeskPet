...
private:
  // Yes, everything is currently still accessible. Be responsible and don't
  // mess things up :)
...
public:
  ...
  int eyeLxDefault = ((screenWidth) - (eyeLwidthDefault + spaceBetweenDefault +
                                       eyeRwidthDefault)) /
                     2;
  ...
  void drawEyes() {
    ...
    eyeRyNext = eyeLyNext; // right eye's y position should be the same as for
                           // the left eye
    ...
    if (confused) {
      if (confusedToggle) {
        ...
        confusedToggle = 0;
                 ...
                 confusedAnimationTimer + confusedAnimationDuration) {
        setHFlicker(0, 0);
        confusedToggle = 1;
        confused = 0;
      }
