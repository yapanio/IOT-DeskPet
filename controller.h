...
private:
  ...
  unsigned long lastTimeBuzzerSounded =
      0; // Tracks interval between warning beeps
...
public:
  Controller()
      : sensors(DHT_PIN), servo(SERVO_PIN), actuators(LED_PIN, BUZZER_PIN) {}
  ...
  void update() {
    ...
    blynk.update(sensors.getTemperature(), sensors.getHumidity(),
                 sensors.getHeatIndex(), sensors.getLightLux(),
...
private:
  ...
  void evaluateState() {
    ...
    if (temp > 42.0) {
      ...
      if (now - lastTimeLightChecked > 600000) {
        ...
        nextState =
            (currentState == WARNING_DARK) ? WARNING_DARK : NORMAL_HAPPY;
      }
    } else {
  ...
  void handleSingleTap() {
    ...
    if (testModeActive) {
      ...
      case WARNING_DARK:
      default:
        sensors.setMock(false, 0, 0,
                        0); // Disable mock and return to physical sensors
        Serial.println(
            F("[Test Mode] Disabling Mock (Returning to NORMAL_HAPPY)."));
        break;
      }
      alarmMuted = false;
    ...
    bool alarmSounding = !alarmMuted && (currentState == DANGER_FIRE ||
  ...
  void handleTripleTap() {
    ...
    danceEndTime =
        millis() + 5000; // Limit this specific dance gesture to 5 seconds
  }
  ...
  void updateTouch() {
    ...
    if (!isTouched && lastTouchState) {
      ...
      // Register tap if released within normal duration and not part of a long
      // press
    ...
    // Dispatch tap callbacks after a short timeout (400ms gap after last touch
    // release)
